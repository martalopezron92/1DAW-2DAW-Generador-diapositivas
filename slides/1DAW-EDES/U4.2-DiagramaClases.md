...
class Cliente {
  ...
  + contratarMembresia(tipo: TipoMembresia): Membresia
  + reservarClase(clase: ClaseGrupal): Reserva
...
class ClaseGrupal {
  ...
  + asignarInstructor(instructor: Instructor): void
  + hayPlazasDisponibles(): boolean
  + agregarReserva(reserva: Reserva): void
  + registrarAsistencia(cliente: Cliente): Asistencia
}
...
class Instructor {
  ...
  - nombre: String
  - especialidad: String
  - horariosDisponibilidad: List<String>

  + estaDisponible(horario: LocalDateTime): boolean
}
...
Cliente "1" o-- "0..*" Membresia : contrata
Cliente "1" -- "0..*" Reserva : realiza
ClaseGrupal "1" o-- "0..*" Reserva : gestiona

Instructor "1" -- "0..*" ClaseGrupal : imparte

Reserva "1" *-- "0..1" Asistencia : genera
